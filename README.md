...
마지막 Make 실습은 앞선 파이썬 크롤링 실습과 같은 결과물(Hacker News 크롤링 → Gemini 요약 → Telegram 전송)을 코드 없이 만드는 비교 실습입니다. HTTP → Text parser → Text aggregator → Gemini AI → Telegram Bot 5개 모듈의 설정값을 화면 항목 그대로 정리하고, 무료 플랜 credit 소진을 막는 실행 주기 주의사항과 증상별 문제 해결 표를 함께 제공합니다.
